...
public class CustomizerCommand extends CustomizerBase {
    ...
    public void executeCustomGlobalFunction(CustomizerBase cb, Player player, Event event, String line, String fc, String arg, String[] args) {
        if (fc.equals("@CMD")) {
            ...
            if (CommandManager.getIt().isRegistered(cmdlabel))
                CustomizerExecutor.executeScript(this, player, CustomizerScript.getScript(CommandManager.getIt().getScript(cmdlabel)), event, arg);
            else
    ...
    public void form_checkCmd(PlayerFormRespondedEvent event, Object data) {
        ...
        str += CustomizerScript.getScript(CommandManager.getIt().getScript(label)).getLinesString();
        ...
        if (e instanceof PlayerCommandPreprocessEvent) {
            ...
            if (event.getMessage().startsWith("/")) {
                ...
                if (CommandManager.getIt().isRegistered(label)) {
                    CustomizerExecutor.executeScript(this, event.getPlayer(), CustomizerScript.getScript(CommandManager.getIt().getScript(label)), event, event.getMessage().substring(1).trim());
                    event.setCancelled();
                }
            }
        }
