...
public class CustomizerDialog extends CustomizerBase {
	...
	public void customizer_dialog_receiver(PlayerFormRespondedEvent event, Object data) {
		if (event.getWindow() instanceof FormWindowModal) {
			...
			String script = "";
			if (response.getClickedButtonId() == 0)
				script = dialog.getAScript();
			else if (response.getClickedButtonId() == 1)
				script = dialog.getBScript();

			CustomizerExecutor.executeScript(this, event.getPlayer(), script, CustomizerScript.getSources(script), event, new Object[] { event, data });
		} else if (event.getWindow() instanceof FormWindowSimple) {
			DialogWindowSimple dialog = (DialogWindowSimple) data;
			FormResponseSimple response = (FormResponseSimple) event.getResponse();

			CustomizerExecutor.executeScript(this, event.getPlayer(), dialog.getScript(response.getClickedButtonId()), CustomizerScript.getSources(dialog.getScript(response.getClickedButtonId())), event, new Object[] { event, data });
		} else if (event.getWindow() instanceof FormWindowCustom) {
			DialogWindowCustom dialog = (DialogWindowCustom) data;
			FormResponseCustom response = (FormResponseCustom) event.getResponse();

			CustomizerExecutor.executeScript(this, event.getPlayer(), dialog.getScript(), CustomizerScript.getSources(dialog.getScript()), event, response.getResponses());
		}
	}
...
////////////////////////////////////////
	...
	public void form_dialog_in(PlayerFormRespondedEvent event, Object data) {
		...
		if (!DialogManager.isDialogExist(name)) {// 다이어로그 생성
			...
			if (DialogManager.getIt().getType(name) == D_Type.FormWindowModal) {
				...
				window.addElement(new ElementInput("1번버튼 스크립트", "", dialog.getAScript()));
				window.addElement(new ElementInput("2번버튼 텍스트", "", dialog.getBText()));
				window.addElement(new ElementInput("2번버튼 스크립트", "", dialog.getBScript()));
	...
	public void form_dialog_st_1(PlayerFormRespondedEvent event, Object data) {
		...
		if (D_Type.get(type) == D_Type.FormWindowModal) {
			...
			window.addElement(new ElementInput("1번버튼 스크립트"));
			window.addElement(new ElementInput("2번버튼 텍스트"));
			window.addElement(new ElementInput("2번버튼 스크립트"));
			SimpleDialog.sendDialog(this, "form_dialog_set_modal", event.getPlayer(), window, name);
	...
	public void form_dialog_set_modal(PlayerFormRespondedEvent event, Object data) {
		...
		String at = ((FormResponseCustom) event.getResponse()).getInputResponse(2);
		String as = ((FormResponseCustom) event.getResponse()).getInputResponse(3);
		String bt = ((FormResponseCustom) event.getResponse()).getInputResponse(4);
		String bs = ((FormResponseCustom) event.getResponse()).getInputResponse(5);

		DialogWindowModal dialog = (DialogWindowModal) DialogManager.getIt().getDialog((String) data, D_Type.FormWindowModal);
		dialog.setTitle(title);
		dialog.setContext(context);
		dialog.setAText(at);
		dialog.setAScript(as);
		dialog.setBText(bt);
		dialog.setBScript(bs);
		dialog.save();
...
class DialogWindowModal extends DialogWindow {
	...
	public DialogWindowModal(String name) {
		...
		if (cfg != null) {
			...
			setAScript(cfg.getString("A_Script"));
			setBText(cfg.getString("B_Text"));
			setBScript(cfg.getString("B_Script"));
		}
	}
	...
	public void setAScript(String script) {
		AScript = script;
	}
	...
	public void setBScript(String script) {
		BScript = script;
	}
	...
	public void save() {
		...
		cfg.set("A_Script", getAScript());
		cfg.set("B_Text", getBText());
		cfg.set("B_Script", getBScript());
